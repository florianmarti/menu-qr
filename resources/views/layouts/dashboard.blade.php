        ...
        >
            <div class="flex items-center justify-center h-20 gap-3 px-6 border-b border-gray-700">
                @if (Auth::user()->isAdmin())
                    <i class="text-2xl fas fa-shield-halved text-blue-400"></i>
                    <span class="text-xl font-semibold">{{ __('Panel Admin') }}</span>
                @else
                    {{-- Lógica del Cliente (Dueño de Restaurante) --}}
                    @if(Auth::user()->restaurant->logo_path)
                        <img src="{{ Storage::url(Auth::user()->restaurant->logo_path) }}" alt="Logo" class="h-12 w-auto object-contain">
                    @else
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                @else
                    @include('layouts.partials._customer-nav')
                @endif
            </nav>
